...
const apiPath = {
  ...
  fetchTrendingTv: `${url}/trending/tv/week?api_key=${apiKey}`,
};
...
async function fetchTrendingTv() {
  ...
  const res = await fetch(apiPath.fetchTrendingTv);
  // console.log(res);
  ...
  try {
    ...
    if (Array.isArray(categories) && categories.length > 0) {
      categories.forEach((cat) => {
        fetchAdndBuildTvSection(cat.id, cat.name);
      });
    }
    // console.log(data);
  } catch (error) {
...
async function fetchAdndBuildTvSection(id, name) {
  try {
    const res = await fetch(apiPath.fectchTvList(id));
    const data = await res.json();
    const tv = data.results;
    // console.log(tv);
    if (Array.isArray(tv) && tv.length > 0) {
      buildTvSection(tv, name);
    }
  } catch (error) {
    console.log(error);
  }
...
function buildTvSection(tv, categoriName) {
